...
workflow guppy_basecalling {
    ...
    call splitFast5s {
        input:
        file_to_split_tar = fast5_tar_file,
    ...
    output {
        File bam = concatenateBam.concatenatedBam
        File fastq = concatenateFastq.concatenatedFastq
        File summary = concatenateSummary.concatenatedSummary
    }
}
...
task concatenateSummary {
    ...
    output {
        File concatenatedSummary = "${sample_name}_${guppy_version}_sequencing_summary.txt.gz"
    }
